...
class poloniex:
        ...
        if (command == "returnTicker" or command == "return24Volume"):
            ...
            ret = urllib2.urlopen(urllib2.Request(
                'https://poloniex.com/public?command=' + "returnChartData" + '&currencyPair=' + str(
                    req['currencyPair']) + "&start=" + str(req["start"]) + "&end=" + str(req["end"]) + "&period=" + str(req["period"])))
            return json.loads(ret.read())
        else:
            req['command'] = command
    ...
    # Outputs:
    # orderNumber   The order number
    def buy(self, currencyPair, rate, amount, fillOrKill, postOnly):
        return self.api_query('buy', {"currencyPair": currencyPair, "rate": rate, "amount": amount, "fillOrKill": fillOrKill, "postOnly": postOnly})
    ...
    # Outputs:
    # orderNumber   The order number
    def sell(self, currencyPair, rate, amount, fillOrKill, postOnly):
        return self.api_query('sell', {"currencyPair": currencyPair, "rate": rate, "amount": amount, "fillOrKill": fillOrKill, "postOnly": postOnly})
